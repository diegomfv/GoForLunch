...
//        editor.apply();
    ...
    private void doAPIRequest() {
        ...
        final GooglePlaceWebAPIService clientPhoto = Common.getGooglePlacePhotoApiService();
        ...
        for (int i = 0; i < restaurants.size(); i++) {
            ...
            Call<PlaceById> callPlaceId = clientPlaceId.fetchDataPlaceId(restaurants.get(i).getPlaceId(), "AIzaSyDuv5PtP5uwugkDW189v9_ycrp8A0nlwkU");
            callPlaceId.enqueue(new Callback<PlaceById>() {
                ...
                public void onResponse(Call<PlaceById> call, Response<PlaceById> response) {
                    ...
                    /** We update the object in the database
                     * */
                    AppExecutors.getInstance().diskIO().execute(new Runnable() {
                        @Override
                        public void run() {
                            Log.d(TAG, "run: Updating restauranttotry: " + result.getPlace_id());
                    ...
                    if (result.getPhotos() != null) {
                        ...
                        final Call<String> callPhoto = clientPhoto.fetchDataPhoto("800" , photo[0].getPhoto_reference(), "AIzaSyDuv5PtP5uwugkDW189v9_ycrp8A0nlwkU");
                        callPhoto.enqueue(new Callback<String>() {
                            ...
                            public void onResponse(Call<String> call, Response<String> response) {
                                ...
                                AppExecutors.getInstance().diskIO().execute(new Runnable() {
                                    @Override
                                    public void run() {
                                        Log.d(TAG, "run: updating photo_/image_url");
                                        mDb.restaurantDao().updateRestaurantPhoto(result.getPlace_id(), photo_url);
                                    }
                                });
                    ...
                    Call <MatrixDistance> callMatrix = clientMatrix.fetchDistance("imperial", "place_id:" + result.getPlace_id(), myPosition, "AIzaSyCebroRUS4VPwvDky6QXHoNfEr0bPHKkYc" );
                    callMatrix.enqueue(new Callback<MatrixDistance>() {
                        ...
                        public void onResponse(Call<MatrixDistance> call, Response<MatrixDistance> response) {
                            ...
                            AppExecutors.getInstance().diskIO().execute(new Runnable() {
                                ...
                                public void run() {
                                    Log.d(TAG, "run: updating photo_/image_url");
                                    mDb.restaurantDao().updateRestaurantDistance(result.getPlace_id(), distanceValue);
                                }
                            });
                    ...
                    //https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=place_id:ChIJo8eWxtqNcUgRqzOCyl2ECWg&destinations=51.457126,-2.606165&key=AIzaSyCebroRUS4VPwvDky6QXHoNfEr0bPHKkYc
